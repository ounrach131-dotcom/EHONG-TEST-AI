...
export default function App() {
  const [sessions, setSessions] = useState<Record<string, ChatSession>>(() => {
    ...
    let sessionData: Record<string, ChatSession> = saved ? JSON.parse(saved) : {};

    // Ensure all predefined models exist, but keep user messages if they already had a session
    INITIAL_MODELS.forEach(m => {
      if (!sessionData[m.id]) {
        sessionData[m.id] = {
          id: m.id,
          model: m,
          messages: [{ id: 'intro', senderId: m.id, text: `Unit ${m.name} synchronized. All protocols active.`, timestamp: Date.now() }],
        };
      } else {
        // Refresh model metadata but keep history
        sessionData[m.id].model = { ...m, ...sessionData[m.id].model };
      }
    });
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.start();
      setIsRecording(true);
    } catch (err) { alert("Microphone required for neural transcription."); }
  };
  ...
  const handleDeleteSession = (id: string) => {
    if (!confirm("Destroy this neural unit and all its message history?")) return;
  ...
  const handleSaveModel = () => {
    ...
    const model: AIModel = { id, name: modelName, avatar: modelAvatar, description: 'Custom Intelligence Unit', systemInstruction: modelInstruction, status: 'online' };
    setSessions(prev => ({
      ...prev,
      [id]: { id, model, messages: isNew ? [{ id: 'intro', senderId: id, text: `Unit ${modelName} deployed. Syncing logs...`, timestamp: Date.now() }] : prev[id].messages }
    }));
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-200 overflow-hidden font-sans select-none">
      <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => {
         ...
         if (file) sendMessage(`[File Sent: ${file.name}]`);
      }} />
      
      {/* Navigation - Unit List */}
      <aside className="w-20 md:w-80 bg-slate-900/40 border-r border-slate-800 flex flex-col backdrop-blur-3xl shrink-0">
        <div className="p-6 space-y-4">
          <div className="flex justify-between items-center">
            <h1 className="hidden md:block text-2xl font-[900] text-[#8b92ff] tracking-tighter uppercase italic leading-none">AI NEXUS</h1>
            <button onClick={() => { setEditingModelId(null); setModelName(''); setModelInstruction(''); setIsModelModalOpen(true); }} className="w-10 h-10 bg-indigo-600/10 hover:bg-indigo-600/20 rounded-xl flex items-center justify-center border border-indigo-500/30 transition-all hover:scale-105">
              <span className="text-xl">+</span>
            </button>
          </div>
          <div className="hidden md:block">
            <input type="text" placeholder="Filter Units..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-800/30 border border-slate-700/50 rounded-xl py-2 px-4 text-xs outline-none focus:ring-1 focus:ring-indigo-500 transition-all" />
          </div>
        </div>
        <nav className="flex-1 overflow-y-auto px-3 space-y-1 custom-scrollbar">
          {filteredSessions.map(session => (
            <div key={session.id} className="group relative">
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all">
                <button onClick={(e) => { e.stopPropagation(); handleEditSession(session); }} className="p-1.5 bg-slate-800 border border-slate-700 rounded-lg text-indigo-400 hover:text-white hover:bg-slate-700 shadow-xl">✎</button>
                <button onClick={(e) => { e.stopPropagation(); handleDeleteSession(session.id); }} className="p-1.5 bg-slate-800 border border-slate-700 rounded-lg text-red-400 hover:text-white hover:bg-slate-700 shadow-xl">✕</button>
              </div>
            </div>
          ))}
        </nav>
        <div className="p-4 bg-slate-900/40 border-t border-slate-800 flex items-center gap-2 justify-center">
            <div className={`w-1.5 h-1.5 rounded-full ${syncStatus === 'syncing' ? 'bg-indigo-400 animate-pulse' : 'bg-green-500 shadow-sm shadow-green-500/50'}`}></div>
            <span className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-500">{syncStatus === 'syncing' ? 'Syncing...' : 'Encrypted & Synced'}</span>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative bg-[#020617]">
        <header className="h-20 bg-slate-900/60 backdrop-blur-xl border-b border-slate-800 flex items-center justify-between px-8 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-black text-white text-lg tracking-tight uppercase leading-none mb-1">{activeSession.model.name}</h2>
              <div className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${isThinking ? 'bg-indigo-400 animate-pulse' : 'bg-green-500'}`}></div>
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">{isThinking ? 'Syncing Data...' : 'Terminal Active'}</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-[10px] text-indigo-400 font-black uppercase tracking-widest hidden lg:block">Tactical Mode</span>
            <select value={selectedStrategy} onChange={(e) => setSelectedStrategy(e.target.value as StrategyType)} className="bg-slate-800 border border-slate-700 text-[10px] font-black text-white rounded-xl px-4 py-2 outline-none uppercase tracking-widest cursor-pointer hover:border-indigo-500/50 transition-all">
              {Object.values(StrategyType).map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        </header>

        {/* Chat Feed - Historical SMS logs for this account */}
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                <div className={`max-w-[80%] group relative flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  {/* SMS Tactical Overlays */}
                  <div className={`absolute -top-7 ${isMe ? 'right-0' : 'left-0'} flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all z-10`}>
                    {!msg.isSticker && (
                      <button onClick={() => {setEditingMessageId(msg.id); setEditMessageText(msg.text);}} className="px-2 py-0.5 bg-slate-800 border border-slate-700 rounded-md text-[9px] font-black text-slate-400 hover:text-indigo-400 uppercase transition-colors">Edit</button>
                    )}
                    <button onClick={() => {
                        setSessions(prev => ({ 
                          ...prev, 
                          [activeChatId]: { ...prev[activeChatId], messages: prev[activeChatId].messages.filter(m => m.id !== msg.id) } 
                        }));
                    }} className="px-2 py-0.5 bg-slate-800 border border-slate-700 rounded-md text-[9px] font-black text-slate-400 hover:text-red-400 uppercase transition-colors">Delete</button>
                    {!isMe && !msg.isSticker && (
                      ...
                      }} className="px-2 py-0.5 bg-indigo-600/20 border border-indigo-500/30 rounded-md text-[9px] font-black text-indigo-400 hover:text-white uppercase transition-colors">
                        {analyzingId === msg.id ? 'Thinking...' : '✨ Cheat'}
                      </button>
                    )}
                  </div>
                      ...
                      {editingMessageId === msg.id ? (
                        <div className="space-y-3 min-w-[200px]">
                          ...
                          <div className="flex justify-end gap-3 text-[10px] font-black uppercase tracking-widest">
                             <button onClick={() => setEditingMessageId(null)} className="text-slate-400 hover:text-white">Cancel</button>
                             <button onClick={() => {
                             ...
                             }} className="text-indigo-400 hover:text-white">Update</button>
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className="mt-2 text-[9px] font-black opacity-30 uppercase tracking-[0.2em]">{new Date(msg.timestamp).toLocaleTimeString()}</div>
                </div>
              </div>
            );
          })}
          {isThinking && <div className="text-[10px] text-slate-500 uppercase font-black tracking-[0.4em] animate-pulse">Computing Strategy...</div>}
        </div>
        ...
        <footer className="p-8 bg-slate-900/40 backdrop-blur-md border-t border-slate-800 relative">
          ...
          {isAttachMenuOpen && (
             <div className="absolute bottom-full left-8 mb-4 bg-[#1e2733] border border-[#2b3949] rounded-2xl shadow-2xl w-64 flex flex-col overflow-hidden z-30 animate-in slide-in-from-bottom-4 duration-300">
                ...
                <button onClick={() => setIsAttachMenuOpen(false)} className="flex items-center gap-4 px-6 py-4 hover:bg-[#2b3949] transition-all group border-b border-white/5">
                    ...
                    <span className="text-sm font-bold text-slate-200">Neural Poll</span>
                </button>
                <button onClick={() => setIsAttachMenuOpen(false)} className="flex items-center gap-4 px-6 py-4 hover:bg-[#2b3949] transition-all group">
                    ...
                    <span className="text-sm font-bold text-slate-200">Neural Location</span>
                </button>
             </div>
          )}

          {/* Asset Picker (Emoji/Stickers) */}
          {isPickerOpen && (
            <div className="absolute bottom-full left-8 mb-4 bg-slate-900 border border-slate-800 rounded-[2.5rem] shadow-2xl w-80 flex flex-col overflow-hidden z-30 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex border-b border-slate-800 text-[10px] font-black uppercase tracking-[0.2em]">
                {(['Emoji', 'Stickers', 'GIFs'] as PickerTab[]).map(tab => (
                  <button key={tab} onClick={() => setPickerTab(tab)} className={`flex-1 py-4 transition-all ${pickerTab === tab ? 'text-indigo-400 bg-indigo-600/5' : 'text-slate-500 hover:bg-white/5'}`}>{tab}</button>
                ))}
              </div>
          ...
          <div className="flex gap-4 items-end max-w-5xl mx-auto">
            <button onClick={() => { setIsAttachMenuOpen(!isAttachMenuOpen); setIsPickerOpen(false); }} className={`w-14 h-14 rounded-2xl flex items-center justify-center border shrink-0 transition-all ${isAttachMenuOpen ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-indigo-400 hover:border-indigo-500/50'}`}>
        ...
        {analysisPanel && (
          <div className="absolute inset-y-0 right-0 w-full md:w-[480px] bg-slate-950/98 backdrop-blur-3xl border-l border-indigo-500/30 shadow-2xl z-20 flex flex-col animate-in slide-in-from-right duration-500">
            <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-indigo-950/10">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-indigo-600 rounded-[1.2rem] flex items-center justify-center text-white shadow-xl shadow-indigo-600/30 font-black text-2xl tracking-tighter">✨</div>
                <div>
                  <h3 className="font-black text-white text-xl tracking-tighter uppercase leading-none">Strategic Insight</h3>
                  <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mt-1">Live Intelligence Feed</p>
                </div>
              </div>
              <button onClick={() => setAnalysisPanel(null)} className="p-2 text-slate-500 hover:text-white transition-all bg-slate-800/50 rounded-lg border border-slate-700">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
              <div className="space-y-6">
                <div className="bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-800 shadow-inner">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">Message Vibe</h4>
                  <p className="text-white text-3xl font-black italic tracking-tighter leading-none">{analysisPanel.analysis.vibe}</p>
                </div>
                <div className="bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-800 shadow-inner">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">Tactical Analysis</h4>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {analysisPanel.analysis.suggestedReplies.map((reply, i) => (
                    <button key={i} onClick={() => {setInputText(reply.text); setAnalysisPanel(null);}} className="w-full text-left p-6 bg-slate-900/80 hover:bg-indigo-600/10 border border-slate-800 hover:border-indigo-500/50 rounded-[2rem] transition-all group active:scale-95">
                      <p className="text-white font-bold mb-2 text-sm leading-snug">{reply.text}</p>
                      <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest border-t border-white/5 pt-3 mt-3 group-hover:text-indigo-300 transition-colors">{reply.explanation}</p>
                    </button>
                  ))}
                </div>
              </div>
        ...
        {/* Deploy Custom Intelligence Modal */}
        {isModelModalOpen && (
          <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-50 flex items-center justify-center p-6 transition-all">
            <div className="bg-slate-900 w-full max-w-xl rounded-[4rem] border border-slate-800 p-12 shadow-[0_0_100px_rgba(79,70,229,0.1)] animate-in zoom-in-95 duration-500">
              <div className="flex items-center gap-6 mb-12">
                <div className="w-16 h-16 bg-slate-800 border border-slate-700 rounded-[1.8rem] flex items-center justify-center text-5xl shadow-inner">{modelAvatar}</div>
                <div>
                  <h3 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">
                    {editingModelId ? 'Re-Sync Unit' : 'Deploy Unit'}
                  </h3>
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mt-2">Neural Core Mapping</p>
                </div>
              </div>
              <div className="space-y-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-8">Unit ID / Name</label>
                  <input type="text" value={modelName} onChange={(e) => setModelName(e.target.value)} placeholder="e.g. Tactical Architect" className="w-full bg-slate-800/50 border border-slate-700 rounded-[1.5rem] p-6 text-white outline-none focus:border-indigo-500 transition-all font-bold shadow-inner" />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-8">Core Logic & Directives</label>
                  <textarea rows={4} value={modelInstruction} onChange={(e) => setModelInstruction(e.target.value)} placeholder="Program the AI persona and decision rules..." className="w-full bg-slate-800/50 border border-slate-700 rounded-[1.5rem] p-6 text-white outline-none resize-none focus:border-indigo-500 transition-all font-medium leading-relaxed shadow-inner" />
                </div>
                <div className="flex gap-6 pt-6">
                  <button onClick={() => { setIsModelModalOpen(false); setEditingModelId(null); }} className="flex-1 py-6 text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] hover:text-white transition-colors">Abort</button>
                  <button onClick={handleSaveModel} disabled={!modelName} className="flex-[2] py-6 bg-indigo-600 text-white rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.4em] disabled:opacity-50 shadow-2xl shadow-indigo-600/30 hover:bg-indigo-500 transition-all active:scale-95">
                    {editingModelId ? 'Sync System' : 'Deploy System'}
                  </button>
                </div>
              </div>
            </div>
